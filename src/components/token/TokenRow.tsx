...
/** Mini sparkline SVG — real data only, no fake noise */
function Sparkline({ token }: { token: TokenPair }) {
  const points = useMemo(() => {
    ...
    // 5 anchor points from real price change data
    ...
    const range = max - min;
    // Flat line when no real price movement
    if (range === 0) {
      return anchors.map((_, i) => ({
        x: (i / (anchors.length - 1)) * 110,
        y: 16,
      }));
    }
    ...
    >
        ...
        <td className="px-3 py-3 overflow-hidden">
          <div className="flex items-center gap-2 whitespace-nowrap">
            ...
            <div className={`w-8 h-8 rounded-full bg-xdex-card border border-xdex-border flex items-center justify-center flex-shrink-0 ${token.baseToken.imageUrl ? 'hidden' : ''}`}>
              <span className="text-[13px] font-bold text-xdex-accent">{token.baseToken.symbol.charAt(0)}</span>
            </div>

            <div className="min-w-0">
              <div className="flex items-center gap-1.5">
                <span className="font-semibold text-base text-white">{token.baseToken.symbol}</span>
                {token.isVerified && <span className="text-xdex-accent text-[11px]" title="Verified">&#10003;</span>}
                <span className="text-[13px] text-xdex-text-secondary truncate">{token.baseToken.name}</span>
                <span className="text-xdex-text-muted text-[13px]">/</span>
                <span className="text-[13px] text-xdex-text-muted">{token.quoteToken.symbol}</span>
                {isLowLiquidity && <span title="Low liquidity" className="flex-shrink-0"><AlertTriangle size={11} className="text-xdex-yellow/70" /></span>}
                {isNew && <span className="text-[10px] px-1 py-0.5 rounded bg-xdex-accent/15 text-xdex-accent font-semibold flex-shrink-0">NEW</span>}
              </div>
              <div className="flex items-center gap-1.5 mt-0.5">
                <span className="text-xs text-xdex-text-muted">{formatAge(token.createdAt)}</span>
              </div>
            </div>
          </div>
        </td>
      ...
      {show('price') && (
        <td className="px-2 py-3 text-right">
          <div className="text-base text-white font-mono font-medium">{formatPrice(token.priceUsd)}</div>
          <div className={`text-[13px] font-mono font-medium mt-0.5 ${getPercentColor(priceChange)}`}>{formatPercent(priceChange)}</div>
        </td>
      )}
      ...
      {show('txns') && (
        <td className="px-2 py-3 text-right">
          <span className="text-base text-white font-mono">{formatNumber(token.txns24h)}</span>
        </td>
      )}

      {show('liquidity') && (
        <td className="px-2 py-3 text-right">
          <span className="text-base font-mono text-white">{formatUsd(token.liquidity)}</span>
        </td>
      )}

      {show('marketCap') && (
        <td className="px-2 py-3 text-right">
          <span className="text-base text-white font-mono font-medium">{formatUsd(token.marketCap)}</span>
        </td>
      )}
      ...
      {show('safety') && (
        <td className="px-2 py-3 text-center">
          <span
            className={`inline-flex items-center gap-1 text-[11px] font-semibold px-1.5 py-0.5 rounded ${safety.color} ${safety.bgColor}`}
