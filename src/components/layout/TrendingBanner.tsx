...
/**
 * TrendingBanner — DEX-wide trending activity ticker.
 *
 * This component works standalone across ANY XDEX page (Swap, Liquidity, Farm, etc.).
 * It can either:
 *   1. Accept token data via props (when the parent page already fetches pool data)
 *   2. Fetch its own data from the XDEX API (when used on pages that don't have token data)
 *
 * To use DEX-wide: Import this component on any page and render it without props.
 * The banner will auto-fetch trending data from the XDEX API every 60 seconds.
 *
 * In xdex_frontend, add to your shared layout or individual pages:
 *   import TrendingBanner from '@/components/layout/TrendingBanner';
 *   <TrendingBanner />
 *
 * For the Alpha Scan page (which already has token data), pass tokens as props:
 *   <TrendingBanner tokens={currentChainTokens} allTokens={allTokens} onTokenClick={handleClick} />
 */
...
interface TrendingBannerProps {
  /** Pass tokens to avoid refetching (for pages that already have data) */
  tokens?: TokenPair[];
  /** All tokens across chains for banner stats */
  allTokens?: TokenPair[];
  /** Click handler when user clicks a token in the banner */
...
export default function TrendingBanner({ tokens: propTokens, allTokens: propAllTokens, onTokenClick }: TrendingBannerProps) {
  const [scrollOffset, setScrollOffset] = useState(0);
  const [selfFetchedTokens, setSelfFetchedTokens] = useState<TokenPair[]>([]);

  // Self-fetch if no tokens provided (for DEX-wide usage)
  const needsSelfFetch = !propTokens && !propAllTokens;
  useEffect(() => {
    ...
    async function load() {
      try {
        const [x1, sol] = await Promise.allSettled([
          fetchPoolList('x1'),
          fetchPoolList('solana'),
        ]);
        if (cancelled) return;
        const combined = [
          ...(x1.status === 'fulfilled' ? x1.value : []),
          ...(sol.status === 'fulfilled' ? sol.value : []),
        ];
        setSelfFetchedTokens(combined);
      } catch { /* silent */ }
    }
  ...
  const tokens = propTokens || selfFetchedTokens;
  const allTokens = propAllTokens || selfFetchedTokens;
  ...
  return (
    <header className="relative h-10 bg-black border-b border-[#222] overflow-hidden">
      <div
        ...
        style={{ transform: `translateX(-${scrollOffset % 3000}px)` }}
      >
